...
function payinstruction_civicrm_alterContent(&$content, $context, &$tplName, &$object) {
  ...
  if($context == "form") {
    if($tplName == "CRM/Financial/Form/Payment.tpl") {
      ...
      Civi::log()->debug('payinstruction.php::civicrm_alterContent Hook object _attributes class' . '  ' . print_r($object->_attributes["class"], true));
      ...
      // Disable API permission checks
      // Check if selected payment processor is link to this extension
      $manageds = \Civi\Api4\Managed::get(FALSE)
        ->addSelect('module', 'payment_processor_type.class_name')
        ->addJoin('PaymentProcessorType AS payment_processor_type', 'LEFT', ['payment_processor_type.id', '=', 'entity_id'])
        ->addWhere('module', '=', 'me.sxs.payinstruction')
        ->addWhere('payment_processor_type.class_name', '=', $object->_paymentProcessor["class_name"])
        ->execute();
      ...
      if ($manageds->count() == 1) {
        ...
        if ($messageTemplates->count() == 1) {
          foreach ($messageTemplates as $messageTemplate) {
            $pay_instruction = $messageTemplate["msg_html"];
          }
        }  
        
        // Find the div element with id "payment_information"
        $pattern = '/<\s*div\s*id\s*=\s*".*\Qpayment_information\E.*"\s*>\X*?<\s*\/div\s*>/';
        $replacement = '<div id="payment_information">' . $pay_instruction . '</div>';
        // Replace div element with pay_instruction 
        $content = preg_replace ($pattern, $replacement, $content, 1); 
        //Civi::log()->debug('payinstruction.php::civicrm_alterContent Hook modified content' . '  ' . print_r($content, true));
      }
    }
  }   
}
